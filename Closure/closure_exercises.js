...
function cycleIterator(array) {
  ...
  function getNext() {
    const output = array[currentIndex % array.length]
    currentIndex++
    return output
  }
  return getNext
...
// and an output key that contains the result from invoking the passed-in function. HINT: You may need to research how to access information on Date objects.
...
// the returned function will return the same string, except all instances of first strings (of saved pairs) will be replaced with their corresponding second strings
// (of those saved pairs).

function censor() {
  const list = []

  function swapIt(arg, arg2) {
    if (arg2) {
      list.push({ search: arg, replace: arg2 })
      // console.log(list);
    } else {
      list.map((item) => {
        arg = arg.replace(item.search, item.replace)
      })
      return arg
    }
  }
...
// The returned function should return true if the first elements (of each sub-array) being passed into the callback all yield the corresponding second elements
...
// and return that deleted string with the word 'undone' after (separated by a space). If 'undo' is passed into the function and the function's history is empty,
...
// BONUS: Implement blackjack so the DEALER function can return more PLAYER functions that will each continue to take the next number in the array after
// the previous PLAYER function left off. You will just need to make sure the array has enough numbers for all the PLAYER functions.

function blackjack(array) {
  // To keep track of the deck position
  let index = 0
  ...
  function dealer(num1, num2) {
    ...
    function player() {
      ...
      // To check if player cards sum reached 21 or not
      if (sum + array[index % array.length] <= 21) {
